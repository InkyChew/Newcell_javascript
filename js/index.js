...
function playGame() {
  ...
  var onDragNumber = null,
      ...
      isTemp2Temp = false; //Spade>Heart>Diamond>Club
  ...
  function flushRightOrderTop() {
    rightOrderTopDesks.forEach(function (desk, index) {
      ...
      rightOrderTopDeskElem.setAttribute('class', 'top-right-desk d-flex justify-content-center align-items-center'); // var rightOrderTopDeskImg = document.createElement('img');
  ...
  function displaySuccessCard(noCard, card, append) {
    ...
    if (noCard) {
      successCard.setAttribute('src', "assets/images/".concat(card, ".png"));
    } else {
  ...
  function flushUnorderDesks() {
    unorderDesks.forEach(function (desk, index) {
      ...
      desk.forEach(function (card, cardIndex) {
        ...
        unorderCardElem.id = "unorderCard-".concat(cardIndex);
        ...
        if (!isGamePause && cardIndex + 1 == desk.length) {
          ...
          for (var i = cardIndex; i >= 0; i--) {
            ...
            if (cardUpNumber % 13 === cardDownNumber % 13 + 1 && cardUpColor + cardDownColor !== 5 && cardUpColor !== cardDownColor) {
              console.log("multi-draggable" + cardUpNumber + cardDownNumber); // unorderCardElem.draggable = true;
            }
          }
        }
  ...
  function dragStart(e) {
    ...
    console.log(onDragNumber); // if(onDragNumber === unorderDesks[onDragDeskNumber].slice(-1, 0))
  ...
  function dragEnter(e) {
    ...
    if (onDragNumber === onDropNumber) return; // if(!isTempDesk && !isSuccessDesk && e.target.draggable === true){
    //   console.log("over")
  ...
  function dragOver(e) {
    ...
    if (!isTempDesk && !isSuccessDesk) {
      onDropDeskNumber = e.target.deskNumber; // console.log("unorder:" + onDropDeskNumber)
    } //left
  ...
  function dragEnd(e) {
    var dragCardColor = Math.ceil(onDragNumber / 13);
    var dropCardcolor = Math.ceil(onDropNumber / 13);
    ...
    if (isSuccessDesk) {
      ...
      var validFirstSuccessCard = rightOrderTopDesks[0].length === 0 && onDropDeskNumber === 0 && onDragNumber === 1 || rightOrderTopDesks[1].length === 0 && onDropDeskNumber === 1 && onDragNumber === 14 || rightOrderTopDesks[2].length === 0 && onDropDeskNumber === 2 && onDragNumber === 27 || rightOrderTopDesks[3].length === 0 && onDropDeskNumber === 3 && onDragNumber === 40;
      ...
      if (validFirstSuccessCard) {
        ...
        console.log(789456);
        var validSuccessDeskCardPush = dragCardColor === dropCardcolor && (onDragNumber % 13 === onDropNumber % 13 + 1 || onDragNumber % 13 === 0 && onDropNumber % 13 === 12);
    ...
    var validUnorderDeskCardPush = dragCardColor !== dropCardcolor && dragCardColor + dropCardcolor !== 5 && (onDropNumber % 13 === onDragNumber % 13 + 1 || onDropNumber % 13 === 0 && onDragNumber % 13 === 12);
  ...
  var restartElem = document.getElementById('newgame');
